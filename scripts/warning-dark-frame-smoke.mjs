...
const outputDirectory = path.join(repositoryRoot, 'test-results', 'warning-dark-v1');
...
for (const { name, frame } of frames) {
  ...
  if (result.status !== 0) {
    throw new Error(`warning-dark-v1 ${name} frame smoke render failed.`);
  }
}

process.stdout.write(
  `Rendered ${frames.length} warning-dark-v1 smoke frames to ${outputDirectory}\n`,
);
