...
const outputDirectory = path.join(repositoryRoot, 'test-results', 'breaking-red-v1');
...
for (const { name, frame } of frames) {
  ...
  if (result.status !== 0) {
    throw new Error(`breaking-red-v1 ${name} frame smoke render failed.`);
  }
}

process.stdout.write(
  `Rendered ${frames.length} breaking-red-v1 smoke frames to ${outputDirectory}\n`,
);
